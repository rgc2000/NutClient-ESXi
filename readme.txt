...
- Connect via ssh to the hypervisor and type the followind commands
    ...
    cd /tmp
    tar -xzvf NutClient-ESXi-2.8.5-2.7.1.x86_64.tar.gz
    ./upsmon-install.sh
...
- Connect via ssh to the host and type the following command
    ...
    esxcli software vib install -d /tmp/NutClient-ESXi-2.8.5-2.7.1-x86_64-offline_bundle.zip

UPDATE

- Same as install but use ./upsmon-update.sh
- Or use esxcli software vib update -d /tmp/NutClient-ESXi-2.8.5-2.7.1-x86_64-offline_bundle.zip
